...
<template>
    ...
    <InnerBanner class="mb-5" title="Discover Custom-Made Window Furnishings"
        ...
        :bannerImage="banner" />
    <section v-for="category in shop.categories" :key="category.id"  class="mt-5" v-if="shop">
        <div class="container" v-if="CheckProducts(category.name, category.products).length > 0">
            <div class="row align-items-center gx-5" style="row-gap:100px;">
                <div class="col">
                    <h2 class="cat-heading">{{ category.name }}</h2>
                </div>
                <!-- <div class="col text-end" v-if="CheckProducts(category.name, category.products).length > 3">
                    <router-link :to="{ name: 'single-category', params: { slug: category.slug } }">See
                        All</router-link>
                </div> -->
            </div>
            <div class="row gx-5" style="row-gap:100px;">
                <template v-for="product in category.products">
                    <div class="col-md-3" v-if="product.category == category.name && product.type == 'custom'">
                ...
                <div class="col-md-4">
                    <div class="shop-content-item">
                        ...
                        <p>With our simple DIY process, you can do it all when it suits you. No need to book a quote or
                            wait
                            for tradespeople when it’s time to install.
                        </p>
                    </div>
                </div>
                <div class="col-md-4">
